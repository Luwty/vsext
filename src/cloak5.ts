...
export function activate(context: vscode.ExtensionContext) {
	...
	// 这里不能使用顶层 await。异步初始化放在 activate 内部，避免 ESM top-level await 报错。
...
function startCountdown() {
	...
	void statsStore?.recordRunStart();
...
function updateCountdownText() {
	...
	statusBarItem.backgroundColor = undefined;
	updateStatsHover(`当前状态: 倒计时中，剩余 ${formatTime(remainingSeconds)}`);
}

function updateStatsHover(extraLine?: string) {
	...
	hover.appendMarkdown(`暂停次数: ${todayStats?.pauseCount ?? 0}\n\n`);
	hover.appendMarkdown(`运行次数: ${todayStats?.runCount ?? 0}\n\n`);
	hover.appendMarkdown(`运行时长: ${formatDuration(todayStats?.runSeconds ?? 0)}`);
